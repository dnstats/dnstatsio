    ...
    if ans:
        for r in ans:
            ...
            if ' issue ' in r:
                has_caa = True
                issue_count += 1
    return {'caa_issue_count' : issue_count, 'caa_wildcard_count' : wildcard_count, 'caa_has_reporting' : has_reporting, 'caa_allows_wildcard' : allows_wildcard, 'caa_exists' : has_caa}
...
def get_dmarc_stats(ans):
    ...
    return {'dmarc_has_aggregate' : aggregate, 'dmarc_has_forensic' : forensic, 'dmarc_exists' : dmarc, 'dmarc_policy' : policy, 'dmarc_sub_policy' : sub_policy}
